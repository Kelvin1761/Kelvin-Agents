...
- [ ] NBA settled-day archive同Tennis mutable SQLite／daily archive接入各自multi-root reader；唔可以將DB snapshot當live DB，亦唔可以用同一種folder假設硬套四線。→ Verify：online/offline corpus count固定、offline full-eval exit non-zero。
