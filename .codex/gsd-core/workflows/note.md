...
- **Global scope**: `C:/tmp/antigravity-gsd-local/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/tmp/antigravity-gsd-local/.codex/notes/`)
...
2. Glob `C:/tmp/antigravity-gsd-local/.codex/notes/*.md` (if directory exists) — global notes
...
Global (C:/tmp/antigravity-gsd-local/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/tmp/antigravity-gsd-local/.codex/notes/` — works in any directory
