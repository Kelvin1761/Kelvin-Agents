...
class RacingEngine:
    ...
    def analyze_horse(self):
        ...
        # Gently penalise the #1 false-positive pattern: high stability
        # on horses with weak class/track suitability.
        ...
        rank_bonus = self._micro_rank_bonus(matrix_scores, feature_scores)
        rank_score = round(ability_score + place_tightening_bonus + rank_bonus, 4)
    ...
    def _sectional_breakdown(self):
        ...
        # SIP-AU-033: Sectional Realism Cap for Big Fields
        field_size = int(parse_float(self.data.get("field_size") or 0))
        if field_size >= 13:
             pf_score = min(80, pf_score)
             pf_notes.append("大場面段速投射採取保守上限")
    ...
    def _pace_map_score(self):
        ...
        if barrier is not None:
            if barrier <= 2:
                ...
                if distance_m is not None and distance_m <= 1100:
                    score += 2
        ...
        if any(token in tactical for token in ("貼欄", "守位", "省位", "切入")):
            bonus = 2 if style_evidence or style_conf in {"高", "High"} else 1
            score += bonus
        ...
        if dry_midfield and current_bucket in {"front", "back"} and not style_evidence and style_conf not in {"高", "High"}:
            score -= 2
        ...
        if (
            ...
            and score > 68
            and (
                (not style_evidence and style_conf in {"", "未明", "低", "Low"})
    ...
    def _jockey_score(self):
        ...
        if any(token in jockey for token in elite_tokens):
            score += 12
            return score, f"{jockey} 屬高階騎師，騎師分 {clip_score(score):.1f}。", "jockey_name"
        if any(token in jockey for token in solid_tokens):
            score += 6
            return score, f"{jockey} 屬有基本把握嘅一級半騎師，騎師分 {clip_score(score):.1f}。", "jockey_name"
        if "(a)" in jockey or "Fitzgerald" in jockey:
            score += 2
            return score, f"{jockey} 有減磅/新鮮手感因素，騎師分 {clip_score(score):.1f}。", "jockey_name"
        return score, f"{jockey or '騎師資料'} 屬中性配置，騎師分 {clip_score(score):.1f}。", "jockey_name"
    ...
    def _trainer_score(self):
        ...
        return score, f"{trainer or '練馬師資料'} 反映馬房部署基礎，練馬師分 {clip_score(score):.1f}。", "trainer_name"
    ...
    def _jockey_horse_fit_score(self):
        ...
        return score, f"{note}，並結合 stage stats / jockey history / 試閘連貫性消化後，人馬配合評為 {clip_score(score):.1f} 分。", "jockey_trainer_fit+stage_stats+trial_continuity+formguide_jockey_history"
    ...
    def _class_score(self):
        ...
        if "大幅降班" in class_move:
            score += 20
            notes.append("大幅降班，等級優勢極大")
        elif "降班" in class_move:
            score += 12
            notes.append("降班作賽，具備等級優勢")
        elif "大幅升班" in class_move:
            score -= 15
            notes.append("大幅升班，難度增加極大")
        elif "升班" in class_move:
            score -= 8
            notes.append("升班挑戰，考驗級數")
        if class_move == "=":
            notes.append("班次維持不變")
        ...
        return score, "；".join(notes) + f"。級數分 {clip_score(score):.1f}。" if notes else "班次資料未見鮮明傾向，級數分中性處理。", "class_move+record_table"
    ...
    def _weight_score(self):
        ...
        if wet_state in {"soft56", "soft7plus", "heavy"}:
            if weight <= 54.0:
                ...
                score -= 2
        return score, f"今仗負磅 {weight:.1f}kg，負磅分 {clip_score(score):.1f}。", "weight"
    ...
    def _distance_score(self):
        ...
        trends = self._sectional_trends()
        score = 60
        if "⭐最佳 ← 今仗 ✅" in target_line or "最佳 ← 今仗 ✅" in line:
            score = 78
        elif "今仗 ✅" in line:
            score = 72
        elif "← 今仗 ❌" in target_line:
            score = 54
        ...
        if "上升" in trends.get("l400_trend", "") and score >= 60:
            score += 2
        return score, f"路程資料連同距離分佈/L400 趨勢判為 {clip_score(score):.1f} 分。", "engine_line+target_distance_line+sectional_trend"
    ...
    def _track_score(self):
        ...
        if track_stats["places"] > 0:
            track_bonus = min(16, track_stats["places"] * 4 + track_stats["wins"] * 3)
            score += track_bonus
            notes.append("同場/同場同程有實際上名支持")
        if track_stats["starts"] > 0 and track_stats["places"] == 0:
            score -= 4
        ...
        elif self._career_starts() > 0 and family["same_venue_starts"] == 0 and family["same_geometry_starts"] == 0:
            score -= 1
        ...
        if wet_state in {"soft56", "soft7plus", "heavy"} and any(token in style for token in ("前", "跟前")):
            ...
            elif wet_state in {"soft7plus", "heavy"}:
                score -= 3
        ...
        if barrier is not None:
            if track_context.get("inside_advantage") and barrier <= 4:
                ...
                draw_penalty = 4 if track_context.get("distance_band") == "1000-1100m" else 2
                score -= draw_penalty
                notes.append("外檔同今日路段幾何仍有白走風險")
        if track_context.get("fair_track") and not notes:
            notes.append("場地整體偏公平，主要靠自身適性分勝負")
        note_text = "；".join(notes) if notes else "場地資料未見鮮明優劣"
        return score, f"{note_text}。場地/場館適性分 {clip_score(score):.1f}。", "track_family+going_stats+meeting_intelligence+track_profile+wet_track"
    ...
    def _formline_score(self):
        ...
        if "無資料" in text or "未有出賽" in text:
            score = min(score, 60)
        future_win_hits = sum(1 for row in rows if re.search(r"出\s*\d+\s*次:\s*(\d+)\s*勝", row.get("next_result", "")) and int(re.search(r"出\s*\d+\s*次:\s*(\d+)\s*勝", row.get("next_result", "")).group(1)) > 0)
        ...
        if followups["lower"] >= 3 and followups["higher"] == 0:
            score -= 2
        return score, f"賽績線級別與對手後續升降班交叉評為 {clip_score(score):.1f} 分。", "formline+opponent_followups+class_followups"
    ...
    def _consistency_score(self):
        ...
        if recent:
            ...
            if "穩定" in self._sectional_trends().get("pi_trend", ""):
                score += 2
            return score, f"近績前五名次 {top5} 次，並按跑法/PI 穩定度修正，穩定性 {clip_score(score):.1f} 分。", "recent_form+run_style+sectional_trend"
    ...
    def _health_score(self):
        ...
        if "中等偏高" in consumption or "高" in consumption:
            # SIP-AU-040: Recovery logic
            if "高 → 中" in consumption or "高 → 低" in consumption:
                # Recovery trend, skip risk flag
                pass
            else:
                score -= 10
                self.risk_flags.append("high_consumption_load")
        ...
        return score, "；".join(notes) + f"。健康/新鮮感 {clip_score(score):.1f} 分。" if notes else "健康資料一般，按中性處理。", "consumption_summary+record_table"
    ...
    def _confidence_score(self):
        ...
        if self._has_last10_warning():
            score -= 4
        return score, f"可用分析錨點 {anchors}/8，並按 style confidence、source 同 warnings 校正後，信心分 {clip_score(score):.1f}。", "data_coverage+style_meta+warnings"
    ...
    def _micro_rank_bonus(self, matrix_scores, feature_scores):
        ...
        barrier = parse_float(self.horse_data.get("barrier"))
        style = self._running_style()
        # SIP-AU-021: Barrier Positioning (Logic-First: Only bonus if matches style)
        if barrier is not None:
            if barrier <= 4:
                # Synergy: Elite Jockey + Good Barrier
                if matrix_scores.get("jockey_trainer", 60) >= 72:
                    bonus += 0.45
            
            if barrier <= 6 and any(token in style for token in ("前", "跟前", "居中前")):
                bonus += 0.45 
                if matrix_scores.get("jockey_trainer", 60) >= 72:
                    bonus += 0.50
            
            if barrier >= 12:
                # Wide barrier is a penalty unless the horse is a specialized back-runner
                if any(token in style for token in ("後", "中後")):
                    bonus -= 0.15 
                else:
                    bonus -= 0.65 # Increased penalty for non-backrunners
        if matrix_scores.get("jockey_trainer", 60) >= 75 and feature_scores.get("jockey_horse_fit_score", 60) >= 72:
            bonus += 0.35
            
        # SIP-AU-026: Class Standout (Intensified)
        cs = feature_scores.get("class_score", 60)
        ds = feature_scores.get("distance_score", 60)
        if cs >= 78:
            bonus += 1.20 # Increased from 0.80
        elif cs >= 72:
            bonus += 0.50
            
        # SIP-AU-047: Distance-Class Resonance (Logic Anchor)
        if cs >= 75 and ds >= 75:
            bonus += 1.50 # Strong anchor for Top 3
            
        # SIP-AU-053: Polarized Classes (Elite vs Field Separation)
        # Calculate race average ability from context if available, otherwise use 64
        field_summary = self.race_context.get("field_summary", {})
        avg_ability = parse_float(field_summary.get("avg_ability_score", 64))
        ability = feature_scores.get("ability_score", 64)
        
        ability_gap = ability - avg_ability
        if ability_gap >= 5.0:
            bonus += 2.50 # Elite tier separation
        elif ability_gap <= -5.0:
            bonus -= 2.50 # Field filler exclusion
            
        # SIP-AU-027: Distance Proven
        if ds >= 78:
            bonus += 0.80 # Increased from 0.45
        elif ds >= 72:
            bonus += 0.30
            
        # SIP-AU-031: Narrative Sentiment (Follow)
        narrative = str(self.data.get("consumption_summary", "") + self.data.get("notes", ""))
        if any(token in narrative for token in ("Follow", "Impressive", "Strong", "級數見稱")):
            bonus += 0.75  # Increased from 0.55
            
        # SIP-AU-032: Class-Weight Synergy
        class_move = self.data.get("class_move", "=")
        weight = parse_float(self.horse_data.get("weight"))
        if "降班" in class_move and weight is not None and weight <= 55.5:
            bonus += 1.80  # Increased from 1.20
            
        # SIP-AU-036: PI Trend Bonus (Balanced)
        trends = self._sectional_trends()
        pi_trend = trends.get("pi_trend", "")
        l400_trend = trends.get("l400_trend", "")
        
        if "上升軌" in pi_trend or "上升軌" in l400_trend:
            bonus += 1.80  # Normalized from 2.80/4.50
        elif "下跌軌" in pi_trend or "衰退中" in l400_trend:
            bonus -= 1.50  # Decisive but balanced penalty
            
        # SIP-AU-049: Trial Promotion (Balanced)
        ts = feature_scores.get("trial_score", 60)
        if ts >= 82:
            bonus += 1.50 # Normalized from 2.80
        elif ts >= 75:
            bonus += 0.80

        # SIP-AU-058: Elite Reliability (Core Lock)
        stability = matrix_scores.get("stability", 60)
        form_line = matrix_scores.get("form_line", 60)
        if stability >= 75 and form_line >= 75 and not self.risk_flags:
            bonus += 1.50 # Anchor for reliability

        # SIP-AU-061: Form Amnesty (Trial-to-Form Pivot - Relaxed V2)
        if ts >= 75 and form_line < 70:
            bonus += 2.00
            # print(f"DEBUG: [SIP-061] Amnesty for {self.horse_data.get('horse_name')} (+2.0)")

        # SIP-AU-064: Rain Maker (Wet Track Specialization)
        going = str(self.race_context.get("going", "") or "").lower()
        gs = matrix_scores.get("track", 60) # track matrix includes going_score
        if "soft" in going or "heavy" in going:
            if gs >= 75:
                bonus += 2.50 # Proven mudder
            elif gs <= 60:
                bonus -= 1.50 # Unproven or poor mudder

        # SIP-AU-062: Rank 1 Shield (Reliability First)
        # Give extra boost to extremely stable horses to anchor them at Rank 1
        if stability >= 78 and not self.risk_flags:
            bonus += 1.80

        # SIP-AU-060: The "Good" Protector (Risk Cleanup)
        # Penalize unstable or poor-fit horses unless they have elite trials
        fit = feature_scores.get("jockey_horse_fit_score", 60)
        if ts < 80:
            if stability < 55:
                bonus -= 1.50
            if fit < 55:
        ...
        # SIP-AU-050: Sectional Resonance
        ss = feature_scores.get("sectional_score", 60)
        if ss >= 78 and ("上升軌" in pi_trend or "上升軌" in l400_trend):
            bonus += 1.80 # Sectional surge + Rising trend
            
        # SIP-AU-039: Forgiveness Bonus (Hidden Strength)
        f_count = self._forgiveness_count()
        if f_count >= 2:
            bonus += 1.50

        # SIP-AU-043: Reliability Synergy (Rising Star + Consistent)
        stability = matrix_scores.get("stability", 60)
        form_line = matrix_scores.get("form_line", 60)
        
        if ("上升軌" in pi_trend or "上升軌" in l400_trend) and stability >= 70:
            bonus += 1.20

        # SIP-AU-052: The Elite Wall (Gold Anchor)
        if stability >= 75 and form_line >= 75:
            bonus += 2.0 # Strong anchor for Top 3 members
            
        # SIP-AU-045: Track/Distance Specialist
        track_score = matrix_scores.get("track", 60)
        if track_score >= 75:
            bonus += 0.80

        # SIP-AU-037: Risk Penalty (Aggressive)
        if self.risk_flags:
            penalty = 1.50 * len(set(self.risk_flags))
            bonus -= penalty

        # SIP-AU-035: Heavy Track Risk & Bonus
        wet_state = self._wet_state()
        if wet_state in {"heavy", "soft7plus"}:
            going_score = feature_scores.get("going_score", 60)
            if going_score <= 55:
                bonus -= 1.50
            elif going_score >= 75:
                # SIP-AU-044: Mudder/Swimmer Bonus
                bonus += 1.50
    ...
    def _disadvantages(self, feature_scores, matrix_scores):
        ...
        if matrix_scores["class_weight"] <= 55:
    ...
    def _core_logic(self, feature_scores, matrix_scores, advantages, disadvantages):
        ...
        strong = max(matrix_scores, key=matrix_scores.get)
        weak = min(matrix_scores, key=matrix_scores.get)
        style = self._running_style() or "中性"
        opener = self._core_opening(horse_name, strong, matrix_scores[strong], style)
        strength_line = self._core_strength_line(strong, feature_scores, matrix_scores)
        caveat_line = self._core_caveat_line(weak, disadvantages[0], style, matrix_scores)
        close = self._verdict_shape(matrix_scores)
        return f"{opener}{strength_line}{caveat_line}整體睇屬於 {close}。"
    ...
    def _core_opening(self, horse_name, strong_key, strong_score, style):
        label = self._matrix_label(strong_key)
        if strong_score >= 74:
            return f"{horse_name} 今場最值得跟嘅位，首先係 {label}。"
        if strong_key == "stability":
            return f"{horse_name} 呢場唔係靠單一爆點上位，而係勝在近期輪廓未散。"
        if strong_key == "jockey_trainer":
            return f"{horse_name} 今場評價有一部分建基於騎練部署，唔係隨便配搭。"
        return f"{horse_name} 今場整體評分以 {label} 做主軸。"

    def _core_strength_line(self, strong_key, feature_scores, matrix_scores):
        if strong_key == "sectional":
            if feature_scores["distance_score"] >= 72:
                return " 段速表現同今次路程配套算對位，代表佢唔止有速度，連輸出節奏都較容易落地。"
            return " 段速底子本身唔差，只要步速唔走樣，末段保持競爭力嘅機會仍在。"
        if strong_key == "stability":
            return " 近況唔算大起大落，至少唔係靠一次偶然走入前列去撐起評分。"
        if strong_key == "jockey_trainer":
            return " 騎師、馬房同部署配套唔差，臨場發揮比一般中性配搭更值得留意。"
        if strong_key == "form_line":
            return " 過往碰過嘅對手線有一定支撐，令呢份評價唔會只停留喺紙面。"
        if strong_key == "class_weight":
            return " 班次同負磅環境未見特別吃虧，代表佢有空間跑返自己。"
        if strong_key == "track":
            return " 場地條件未必係阻力，對比同場部分對手反而更容易跑回正常水準。"
        return f" 目前最有機會兌現分數嘅，仍然係呢條 {self._matrix_label(strong_key)} 線。"

    def _core_caveat_line(self, weak_key, first_risk, style, matrix_scores):
        label = self._matrix_label(weak_key)
        if weak_key == "race_shape":
            return f" 不過佢要交到最好版本，仍然要靠 {style} 跑法喺步速同走位上搵到順手節奏，否則 {first_risk}。"
        if weak_key == "stability":
            return f" 但狀態線未到可以完全放心嘅級數，{first_risk}。"
        if weak_key == "form_line":
            return f" 問題係賽績線未算好厚，{first_risk}，所以評分上限仍然要留一手。"
        if weak_key == "class_weight":
            return f" 真正要面對嘅關口反而係級數同負重，{first_risk}。"
        if self._career_starts() == 0 and matrix_scores["sectional"] >= 70:
            return f" 唯一要接受嘅係，佢目前主要仍靠備戰同試閘投射，{first_risk}。"
        return f" 同時 {label} 未算完全站穩，{first_risk}。"
    ...
    def _jockey_change_signal(self):
        ...
        if signal:
            return signal
        current = self._clean_identity(self.horse_data.get("jockey"))
        latest_official = self._clean_identity(self.data.get("latest_official_jockey"))
