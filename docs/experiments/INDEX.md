...
- [EXP-20260831-09](EXP-20260831-09-au-unused-leaves-retest-and-sportsbet-sectionals.md) — 九個未入排名 leaf 喺新配置下重測，全部 CI 跨零（NOT SHIPPED）；**但揾到 Sportsbet 頁面一直有逐駒 `Sectionals 600m`（94.1% 覆蓋，已驗證逐駒唔同）同 `In running`（97.5%），從來冇 parse** —— cache 733MB 可零請求回填 998 場做 A/B
