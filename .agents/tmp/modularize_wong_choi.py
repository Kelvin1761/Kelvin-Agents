...
def refactor_agent(target_path, resources_dir):
    ...
    # Check if lines actually reduced
    if len(new_skill_md) < len(lines):
        ...
        result = subprocess.run([sys.executable, safe_writer, "--target", target_path, "--mode", "overwrite", "--content", b64], capture_output=True, text=True)
        print("Updated SKILL.md using P33-WLTM safe file writer.")
        print(result.stdout)


au_target = r'g:\我的雲端硬碟\Antigravity Shared\Antigravity\.agents\skills\au_racing\au_wong_choi\SKILL.md'
au_res = r'g:\我的雲端硬碟\Antigravity Shared\Antigravity\.agents\skills\au_racing\au_wong_choi\resources'
print("Refactoring AU Wong Choi...")
refactor_agent(au_target, au_res)

hkjc_target = r'g:\我的雲端硬碟\Antigravity Shared\Antigravity\.agents\skills\hkjc\hkjc_wong_choi\SKILL.md'
hkjc_res = r'g:\我的雲端硬碟\Antigravity Shared\Antigravity\.agents\skills\hkjc\hkjc_wong_choi\resources'
