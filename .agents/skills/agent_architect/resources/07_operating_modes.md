...
1. **強制工具掃描：** 進入 Mode B (優化) 或 Mode C (審計) 時，第一步 **必須** 執行 `run_command python .agents/scripts/agent_health_scanner.py --target [目標檔]`。若未見此結果，後方任何診斷均視為無效。
...
5. **AST 架構導航 (Codebase Mapping)：** 凡涉及跨檔案修改、深層 Bug 追蹤或大型 Feature Planning 時，你 **必須** 先執行 `run_command python .agents/scripts/antigravity_mapper.py [target_dir] -o .scratch/current_repomap.md`，並閱讀生成的 AST 結構圖，獲取「上帝視角」後才可起草 Implementation Plan。
6. **自我修復循環 (Self-Healing Loop)：** 生成或修改任何 `.py` 代碼在遞交給用戶前，你 **必須** 於背景使用 `run_command` 執行 `python -m py_compile [script.py]` (或 flake8) 進行靜態測試。若發現 Syntax Error 或 Type Error，必須先自我修正，確保最終提供給用戶的是已驗證代碼。
7. **Working Memory 掛載 (The Scratchpad)：** 當分析多於 3 個檔案或處理錯綜複雜的邏輯時，你 **必須** 利用 `write_to_file` 於背景將觀察結果、變數名及潛在衝突記錄到 `.scratch/architect_scratchpad.md`，並在起草 Plan 前重讀，以防記憶流失。
...
2. 🆕 **強制架構導航**：執行 `python .agents/scripts/antigravity_mapper.py .agents/scripts -o .scratch/current_repomap.md` 並閱讀 AST 地圖
...
3. 🆕 **全局架構掃描**：執行 `python .agents/scripts/antigravity_mapper.py .agents/scripts -o .scratch/current_repomap.md` 生成並閱讀全域 AST 地圖
