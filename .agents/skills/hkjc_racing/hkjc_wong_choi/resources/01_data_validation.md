...
為了確保在分析過程中不會因為系統或資源問題而中斷，Wong Choi 在正式啟動 Pipeline 前，必須執行本文件內的掃描。
...
1. `hkjc_wong_choi/SKILL.md`
2. `hkjc_horse_analyst/resources/01_system_context.md`
3. `hkjc_horse_analyst/resources/08_templates_core.md`
...
檢查方法:嘗試呼叫 `list_tables`(SQLite) 或 `read_graph`(Memory)。若失敗，提供修改 `mcp_config.json` 加入 server 的指引給用戶。
