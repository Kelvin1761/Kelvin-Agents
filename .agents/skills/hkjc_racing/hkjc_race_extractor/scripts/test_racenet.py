...
# Debug dumps go to a scratch dir, never the repo — see test_live_sections.py.
# NOTE: Racenet was removed as a data source entirely (76 files deleted); this
# script is kept only as a historical probe and its URLs are long stale.
...
try:
    with urllib.request.urlopen(req) as response:
        ...
        with open(os.path.join(OUT_DIR, 'racenet_overview.html'), 'w', encoding='utf-8') as f:
            f.write(html)
...
try:
    with urllib.request.urlopen(req_print) as response:
        ...
        with open(os.path.join(OUT_DIR, 'racenet_print.html'), 'w', encoding='utf-8') as f:
            f.write(html)
except Exception as e:
