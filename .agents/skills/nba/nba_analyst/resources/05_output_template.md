...
> - 本模板中所有 `[必須填寫具象化理據]` 佔位符都**必須**被替換為真實分析文字
...
> LLM 負責填寫 `[必須填寫具象化理據]` 邏輯欄位 + 審閱 Python 自動核心邏輯。
...
> - LLM 填寫：獨立關卡剖析 (`[必須填寫具象化理據]`)、補充/審閱組合核心邏輯
...
[必須填寫不少於20字的具體數據引證，包含防守匹配分析及球權邏輯]
...
## 📋 自檢
✅ Python 預填完成 | 組合數: [1/2/3/X] | `[必須填寫具象化理據]` 殘留: 0 個 ✔️
