...
### 🔥 高水位正盤膽 (+EV Tier — 組合 1B)
- **基於組合 1A 延伸**：保留 1A 嘅核心穩膽 Leg，但將部分盤口提升到價值線
- **組合最終賠率 (SGM 折扣後) ≥ 3.0 倍**
- 要求每腿勝率 **78%** 以上
- 組合信心 > 8.5
- 重點追求更高 Edge（+EV），而非單純命中率
- **1B vs 1A 對比**：報告中必須明確列出 1B 相對 1A 嘅賠率提升幅度同風險變化
- **建議關數:2-3 Leg（同 1A 關數一致或 +1）**
...
每個組合必須包含:
1. 組合類型標記 (🛡️ 1A / 🔥 1B / 🔥 2 / 💎 3)
2. 關數 (N-Leg)
3. 每個 Leg 嘅完整 `[FILL]` 骨架欄位（見 `05_output_template.md`）
4. 📊 組合分析區塊
5. 完成自檢確認
6. 必須輸出 **4 個組合**（組合 1A + 1B + 2 + 3）
