# NBA Wong Choi Engine Adaptation & Priority Protocols
...
>    ```
> 3. **Session State 持久化**: 嚴禁手寫 `_session_state.md` 造成 Token 浪費及格式破壞。你必須呼叫 `run_command` 執行 `python3 .agents/scripts/session_state_manager.py --update "{TARGET_DIR}" --status "PAUSED" --completed "Game 1-4"` 將進度透過 Python API 自動同步。新 Session 嘅 Session Recovery 同樣會自動偵測。
