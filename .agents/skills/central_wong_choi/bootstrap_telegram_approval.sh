...
BOT="$CANDIDATE_ROOT/.agents/skills/au_racing/au_daily_auto/au_telegram_bot.py"
NOTIFIER="$CANDIDATE_ROOT/.agents/skills/shared_racing/scripts/racing_telegram.py"
...
restore_poller() {
  ...
  launchctl bootstrap "$DOMAIN" "$PLIST" >/dev/null 2>&1 || status=1
  launchctl enable "$DOMAIN/$LABEL" >/dev/null 2>&1 || status=1
  exit "$status"
}
...
for attempt in {1..150}; do
  ...
  status="$(/usr/bin/python3 \
    "$CANDIDATE_ROOT/.agents/skills/central_wong_choi/scripts/central_wong_choi.py" \
    --repo "$CANDIDATE_ROOT" --state-root "$STATE_ROOT" status 2>/dev/null || true)"
  if [[ "$head" == "$SELECTOR"* ]] \
    && print -r -- "$status" | grep -F "Release：${SELECTOR[1,12]} · merged · activate succeeded" >/dev/null; then
    /usr/bin/python3 "$NOTIFIER" --message \
      "✅ 首次 approval handoff 完成 ${SELECTOR[1,12]}；production poller 已接管" \
      --json >/dev/null || true
    exit 0
  fi
  if print -r -- "$status" | grep -F "Release：${SELECTOR[1,12]} · merged · activate failed" >/dev/null; then
