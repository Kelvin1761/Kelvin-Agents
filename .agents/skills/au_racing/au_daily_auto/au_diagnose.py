...
KNOWN = [
    ...
    (r"對應表冇呢個場次",
     ...
     "republish"),
    (r"ETARGET|notarget|No matching version found",
     "npm 裝唔到某個版本（多數係 npx 冇釘版本、攞到當日最新而佢壞咗）",
     "deploy.sh 而家由 package.json 讀 wrangler 版本釘死，唔會再攞最新",
     "republish"),
    (r"/accounts//|UnicodeDecodeError|unbound variable",
     "deploy.sh 自身出錯（變數空、解碼、或者 shell 引號）",
     "run_cmd 改成寬鬆解碼；account ID 解析改用 heredoc；變數後接非 ASCII 加大括號",
     "republish"),
