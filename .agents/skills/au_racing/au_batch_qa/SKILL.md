...
> 3. **「全差」不是有效分析。** 若 🔬 段速法醫 或 ⚡ 形勢與走位 只有單詞摘要而非具體數據 → ❌ FAILED(首出馬除外)。
> 4. **評級矩陣 inline 偵測。** 若 📊 評級矩陣的 8 個維度被壓縮到 ≤3 行 → ❌ FAILED。
> 5. **Top 4 Verdict 格式檢查。** 若 Top 4 的任何位置少於 4 行(馬號+評級+核心理據+風險) → ❌ FAILED。（馬名由 compile 腳本自動填入。）
...
5. ✅ 每個選項都有以下 4 行(exact label matching)?
   - `**馬號及馬名:**` (馬名由 compile 腳本自動填入，LLM 只需提供 horse_num)
   - `**評級與✅數量:**`
   - `**核心理據:**`
   - `**最大風險:**`
6. ✅ 包含 `🎯 Top 2 入三甲信心度` ?
