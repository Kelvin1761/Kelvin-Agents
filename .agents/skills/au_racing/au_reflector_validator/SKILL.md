...
5. **File Writing Protocol**:所有檔案寫入使用 Safe-Writer Protocol (P19v6)：heredoc → /tmp → base64 → safe_file_writer.py。**嚴禁使用 write_to_file / replace_file_content / multi_replace_file_content**（見 Wong Choi P19v6 封殺令）。
...
- **Scripts**:
  - `safe_file_writer.py` — Safe-Writer P19v6 寫入管道
