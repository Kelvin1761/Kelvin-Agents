...
- Six-dimension ranking matrix:
  ...
  - `class_weight`
  - `track`
- Report-only reference dimension:
  - `form_line` (no ranking weight)
...
## Clean Six-Dimension Ranking Contract

- Official ranking uses the static six-dimension weighted score only:
  - `ability_score = pure_7d_score = final_rank_score`
  - `pure_7d_score` and `base_7d_score` retain their legacy names for archive/CSV compatibility; both store the same clean official six-dimension score.
