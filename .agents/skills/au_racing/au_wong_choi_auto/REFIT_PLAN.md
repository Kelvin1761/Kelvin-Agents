...
## 2026-08-05 —— Speedmap 全量 A/B：四個候選全部唔過閘

836 場 Speedmap 抽晒（瀏覽器 + bridge，9 秒節奏，**零失敗零跳過**，約 2 個鐘）。
718 場對齊、94.7% 嘅馬對到預測跑位。行 `au_eval.py` 統一尺：

| 候選 | dev | holdout | Gold | winT3 |
|---|---|---|---|---|
| A 獨立維度 w=0.08 | +0.0008 | +0.0037 · | −0.56 | −0.84 |
| A 獨立維度 w=0.12 | +0.0031 | +0.0047 · | −0.14 | −1.39 |
| **B 取代 `pace_map_score`** | **−0.0209** | −0.0158 | **−2.92** | **−6.55** |
| C 開返 `_pace_bias_adjustment` | +0.0029 | +0.0074 · | −0.84 | +0.00 |

**B 係一個有資訊嘅否定**：用預測跑位取代檔位偏差，Gold −2.92、winT3 −6.55。
即係**檔位偏差比預測跑位更能分辨上名馬**。早前我講「`pace_map_score` 同實際
走位零相關」嗰陣語氣似揭發缺陷 —— 呢個係反證：佢做緊嘅嘢係啱嘅，
只係個名（檔位形勢）容易令人以為佢係跑位預測。**唔應該換。**

A 同 C 分唔開：點估計輕微正，區間跨 0（±0.02 覆蓋唔到 +0.007）。

### ⚠️ 第八個同款缺陷：join 碰撞

第一次跑用 `(日期, 場號)` 對 Speedmap。**同一日有多過一個馬場** ——
100 個場次 / 70 個日期，22 個日子撞期 —— 所以 Flemington 嘅 Race 1 同
Rosehill 嘅 Race 1 撞同一個 key，後者覆蓋前者，另一個馬場攞到錯嘅速度圖。

捉到嘅方法唔係測試，係**數字對唔上**：輸出寫住「有 Speedmap 嘅場次：615」
但出咗 **706 場**。615 個速度圖派畀 706 場，多出嗰 91 場攞緊唔屬於佢哋嘅嘢。
修好之後：836 個速度圖、718 場、對到率由 84.9% 升到 **94.7%**
（對唔上正正就係攞錯咗人哋馬場嘅馬號）。

⚠️ 而污染版嘅場數指標**誇張好多**（Gold −2.93 vs 修好後 −0.84）。
所以我當時觀察到嘅「三個候選 Gold 一致跌 2–3pp」係碰撞造成嘅，唔係真效應。

**可以搬去任何地方嘅一句**：join 之後即刻對數 —— 左邊有幾多、右邊有幾多、
配到幾多。三個數對唔上就係 key 唔夠唯一。

### 咁抽咗嘅嘢係咪白費？

唔係。**報告嘅改善唔靠 A/B 成立**：跑法標籤由 **75% 未知 → 0%**，
`leaders` 由 11.3% 場次非空 → 100%。836 版速度圖已經喺 cache，
接落 Facts 就即刻改善讀者見到嘅嘢。只係**排名唔會郁**，
而呢兩件事唔可以混為一談。
