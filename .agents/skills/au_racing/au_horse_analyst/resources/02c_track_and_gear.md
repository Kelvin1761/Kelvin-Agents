...
- **[AU V4.3 Track Family Confidence]:** 場地適性必須先判斷 `Track Family`，不可只睇 Good/Soft/Heavy 統計。
  - **High confidence:** same venue + same distance/track family + same going band 有 ≥3 場正式賽事，或 ≥2 場正式賽事加一個獨立支持(同類場地試閘、同州同方向、class-par 段速)。
  - **Medium confidence:** same surface/going 但不同 venue，或 same venue 樣本只得 1-2 場。
  - **Low confidence:** 首次急彎、首次直路賽、首次 synthetic、首次跨州、或 trial-only/sire-only 支持。
  - **矩陣封頂:** Low confidence 場地適性最高 ➖；Medium confidence 最高 ✅；只有 High confidence 可判 ✅✅。
  - **Track family 層級:** Same venue > same geometry > same state/direction > same surface/going > sire/trial projection。
...
- **[SIP-RR02] Soft 場地專家雷達 (Soft Track Specialist Radar):**
  ...
  - **能力掃描強化:** Soft 專家即使整體評級不高,仍應在 Underhorse Signal 中標記為「場地能力型」,但不得因場地單一因素覆蓋核心缺陷。
...
- **[SIP-RF01] Soft 入位率雙軌篩選 (Soft Place Rate Dual-Track Screening):**
  ...
  - **效果:**
    ...
    4. **Underhorse Signal:** 觸發 SIP-RF01 且整體評級 B+ 或以下嘅馬匹,列入 🐴`[Soft 入位穩定型]`(場地適應力可作輔助信號,但不自動升級)。
  - **安全閥:**
  ...
  - **邏輯基礎:** 2026-03-28 Rosehill 覆盤中,多匹馬 Soft WR% 偏低但實際 Soft 場穩定入位或贏出。純粹以 Soft WR% 篩選導致引擎低估「場地適應者」嘅穩定性。入位率係場地適應能力嘅第二維驗證——一匹喺 Soft 場穩定跑入前三嘅馬,其場地風險遠低於從未喺 Soft 場入位嘅馬。
...
- **[精英馬房裝備升權 (Elite Stable Gear Premium)]:** 精英馬房(Waller / Snowden / Freedman / Hawkes / Maher / Neasham)嘅首次裝備變動(尤其 Blinkers 1st 或 Tongue Tie 1st)成功率顯著高於平均水平。V4.2 起不再有獨立「裝備與距離」維度；若精英馬房首戴裝備 + 配合強試閘表現,可作為「騎練訊號」嘅 targeted deployment 證據,由 ➖ 升至 ✅(代表裝備變動非探索用途,而係針對性修正)。非精英馬房首次裝備變動維持標準處理(未驗證 = ➖ 最高)。**互斥:** 若同一裝備事件已令「騎練訊號」維度取得 ✅,不可再用 `GEAR_POSITIVE` 微調升級；騎練訊號若已由騎練合作數據或 Quick Backup 間距部署取得 ✅,裝備只可作補充理據。
