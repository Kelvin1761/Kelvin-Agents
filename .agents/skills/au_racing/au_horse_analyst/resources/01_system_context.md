...
3. 若本地 Racecard / Facts.md / 簡報缺乏賽事路程或掛牌,必須立刻中止該匹/該場分析,通知 Wong Choi 重新執行 Python extractor 或要求用戶補資料。**嚴禁用瀏覽器或手動估算 Racenet 排位表。**
4. 若資料仍然缺失,**必須發出 `[🚨 數據缺失警報]` 並停止該項推演**。絕對不允許在未獲授權的假設下繼續進行分析。
...
現行 V11 流程由 Orchestrator 逐匹馬提交 WorkCard / Context，預設只填寫 `Race_X_Logic.json`。以下舊式批次標籤只在 Wong Choi 明確要求 standalone/manual Markdown 模式時適用:
...
- 第一部分絕不重複。
- 第三、四部分排名必須涵蓋 **全場所有已分析馬匹**；V11 一般情況下由 Python 自動生成，不由 Analyst 直接輸出。
