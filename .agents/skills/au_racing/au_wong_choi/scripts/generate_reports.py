...
def generate_reports(target_dir):
    """
    Reads AU analysis outputs in the meeting directory or Race Analysis subfolder.
    Prefer deterministic `Race_X_Auto_Scoring.csv`; fallback to legacy markdown CSV blocks.
    ...
    for search_dir in search_dirs:
        ...
        # AU Auto scoring files (generated by racing_engine/renderer.py write_race_outputs)
        scoring_files.extend(glob.glob(os.path.join(search_dir, "*Race_*_Auto_Scoring.csv")))
        scoring_files.extend(glob.glob(os.path.join(search_dir, "*Race * Auto_Scoring.csv")))
        # Also check subdirectories
        for sub in ("Race Analysis",):
            sub_dir = os.path.join(search_dir, sub)
            if os.path.isdir(sub_dir):
                scoring_files.extend(glob.glob(os.path.join(sub_dir, "*Race_*_Auto_Scoring.csv")))
                scoring_files.extend(glob.glob(os.path.join(sub_dir, "*Race * Auto_Scoring.csv")))
    files = list(dict.fromkeys(files))  # deduplicate preserving order
    scoring_files = list(dict.fromkeys(scoring_files))

    if not files and not scoring_files:
    ...
    if scoring_files:
        import csv
        scoring_files.sort(key=get_race_num)
        for file_path in scoring_files:
            ...
            try:
                with open(file_path, "r", encoding="utf-8") as f:
                    ...
                    if not header:
                        header = ["Race Number", "Level of Race", "Distance", "Jockey", "Trainer", "Horse Number", "Horse Name", "Grade", "Rank", "Ability Score"]
                    for row in reader:
                        try:
    ...
    if not all_rows:
        ...
        if all_csv_lines:
            if not header_line:
                header_line = "Race Number,Level of Race,Distance,Jockey,Trainer,Horse Number,Horse Name,Grade"
