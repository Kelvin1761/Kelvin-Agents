...
- **分析模塊**：正式呼叫 `@au horse analyst` / 啟動大腦逐匹填寫 Orchestrator 指定 JSON 欄位。Speed Map 以 `Facts.md` 內由 `inject_fact_anchors.py` 生成嘅 video commentary + settled-position 模型為準；`au_speed_map_generator.py` 只可作低信心 fallback，不可用 Last10 完賽名次當跑法證據。
