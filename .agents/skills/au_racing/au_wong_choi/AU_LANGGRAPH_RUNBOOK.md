# AU LangGraph Runbook

## Manual Mode

```bash
python .agents/scripts/racing_graph_core.py "<TARGET_MEETING_DIR>" --domain au
```
...
python .agents/scripts/racing_graph_core.py "<TARGET_MEETING_DIR>" --domain au --autopilot
```
...
QA strikes are persisted in `.qa_strikes.json` using the AU race number key, for example `"1"`. Passing final QA resets the strike to `0`.
...
- `trial-only sectional ✅✅`: downgrade or add formal-race sectional evidence.
...
- generate reports before final QA passes.
