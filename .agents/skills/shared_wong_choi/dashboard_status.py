...
def collect_dashboard_status(
    ...
    backup = collect_d1_backup_status(state_root) if state_root is not None else None
    return {
...
def render_dashboard_telegram(payload: dict[str, Any]) -> str:
    ...
    if backup:
        lines.append(
            f"D1 backup：{backup.get('status')} · age {backup.get('age_hours', 'N/A')}h · "
            f"WARM {'係' if backup.get('warm_verified') else '否'} · "
            f"COLD {'係' if backup.get('cold_verified') else '否'}"
        )
    lines.append("Dashboard 只顯示／記帳，唔會自行計 prediction 或改排名。")
    return "\n".join(lines)
