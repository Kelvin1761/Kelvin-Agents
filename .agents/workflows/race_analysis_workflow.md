...
> 
> **以下三個工具已完全封殺，適用於所有 Wong Choi 引擎的所有分析寫入操作：**
> 
> | 封殺工具 | 原因 |
> |---|---|
...
## 另外強制：第二階 Template Injection (Horse Batch Template)

> [!IMPORTANT]
> **各引擎必須讀取自身嘅 Output Template 檔案，唔可以使用其他引擎嘅模板。**
>
> | 引擎 | Output Template 路徑 |
> |---|---|
> | **AU Wong Choi** | `.agents/skills/au_racing/au_horse_analyst/resources/06_templates_core.md` + `06_templates_rules.md` |
> | **HKJC Wong Choi** | `.agents/skills/hkjc_racing/hkjc_horse_analyst/resources/08_output_templates.md` |
>
> **深度約束 (Depth Enforcement)：** 在 `💡 結論與評語` 中的 `核心邏輯`，嚴禁使用過於簡短的總結。必須達到 Wong Choi 的專業長度（約 80-150 字），深度綜合分析「段速、步速形勢、EEM 消耗、騎練意圖及風險」以支持其評級。
...
2. **驗證強制標籤**：確認檔案內包含以下完整且未被修改的標籤字眼：
   ...
   - `🎯 步速崩潰冷門`
   - `🐴⚡ 冷門馬總計`
3. 若發現任何一項缺少或因生成漂移而被更改為其他格式（如原本的 `🥇 **第一選**` 變成了 `首選:`），AI 必須使用 `run_command` + heredoc 管道進行立刻修復（與 P19v6 一致），直到符合為止，絕不能將瑕疵交回給使用者。
