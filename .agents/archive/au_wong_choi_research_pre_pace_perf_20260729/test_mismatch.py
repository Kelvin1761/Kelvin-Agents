"""Archived one-off mismatch finder; executes a full scan on import."""
    ...
    for logic_path in meeting_dir.glob("Race_*_Logic.json"):
        ...
        if rows:
            ...
            for h in sample.get("horses", {}).values():
                name = normalize_horse_name(h.get("horse_name"))
                logic_names.append(name)
                if name in lookup:
                    matched_horses += 1
            
            if matched_horses < 4:
                print(f"FAILED MATCH: {meeting_dir.name} Race {race_no}")
                print("CSV Names:", list(lookup.keys()))
