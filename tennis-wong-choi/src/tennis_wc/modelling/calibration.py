...
def calibrate_sackmann_elo(
    ...
    calibrated over historical matches.
    """
    ...
    for row in rows:
        ...
        overall[winner_id] = winner_overall + k_factor * (1 - expected)
        overall[loser_id] = loser_overall + k_factor * (0 - (1 - expected))

        if surface:
            surface_expected = elo_probability(winner_surface, loser_surface)
            by_surface[winner_id][surface] = winner_surface + k_factor * (1 - surface_expected)
            by_surface[loser_id][surface] = loser_surface + k_factor * (0 - (1 - surface_expected))
