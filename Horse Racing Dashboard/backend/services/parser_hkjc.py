...
# Alternative: capture "### 【No.X】HorseName" or "### 【1】HorseName"
...
# New V9.1 format: **【No.1】 顏色之皇** | ...
HORSE_HEADER_V9_RE = re.compile(
    r'^\*{0,2}【(?:No\.)?(\d+)】\s*(.+?)\*{0,2}\s*\|', re.MULTILINE
)
...
def _parse_jockey_trainer_weight_barrier(header_line: str) -> dict:
    ...
    Or Kelvin new: ### 【1】開心孖寶（檔位:10）| 騎師:艾道拿 / 練馬師:呂健威 | 負磅:135 | 評分:60"""
    result = {}
    
    # Try keyword-based extraction first
    if "騎師" in header_line or "負磅" in header_line:
        jockey_m = re.search(r'騎師[:：]\s*([\u4e00-\u9fffA-Za-z]+)', header_line)
        if jockey_m: result['jockey'] = jockey_m.group(1).strip()
        
        trainer_m = re.search(r'練馬師[:：]\s*([\u4e00-\u9fffA-Za-z]+)', header_line)
        if trainer_m: result['trainer'] = trainer_m.group(1).strip()
        
        weight_m = re.search(r'負磅[:：]\s*(\d+)', header_line)
        if weight_m: result['weight'] = weight_m.group(1).strip()
        
        barrier_m = re.search(r'檔位[:：]\s*(\d+)', header_line)
        if barrier_m: result['barrier'] = int(barrier_m.group(1).strip())
    ...
    result['jockey'] = re.sub(r'\([^)]*\)', '', parts[1]).strip()  # strip (a-2) etc.
    
    idx = 2
    # If the next part is not numeric (e.g. not '135'), it must be the Trainer
    part_stripped = re.sub(r'[磅kg]', '', parts[idx]).strip() if idx < len(parts) else ''
    if idx < len(parts) and not part_stripped.isdigit() and not part_stripped.replace('.', '', 1).isdigit():
        result['trainer'] = re.sub(r'\([^)]*\)', '', parts[idx]).strip()  # strip (C Fownes) etc.
        idx += 1
        
    if idx < len(parts):
        # Strip '磅' suffix: "135磅" -> "135"
        weight_val = re.sub(r'[磅kg]', '', parts[idx]).strip()
    ...
    if idx < len(parts):
        try:
            # Strip '檔' prefix: "檔5" -> 5
            barrier_str = re.sub(r'[檔位]', '', parts[idx]).strip()
...
def _parse_verdict_top_picks(text: str) -> list[TopPick]:
    ...
    for i, hdr in enumerate(headers):
        ...
        # Extract horse number and name — handles: "3 皇龍飛將", "[3] 皇龍飛將", "#8 焦點"
        name_m = re.search(
            r'馬號及馬名[:：]\s*(?:\*\*)?\s*\[?#?(\d+)\]?\s+(.+?)(?:\*\*)?\s*$',
            block, re.MULTILINE
        )
        if not name_m:
            # Alternate: without bold, with optional brackets or hash
...
def _parse_race_header(text: str) -> dict:
    ...
    # Race name — try Markdown Heading first
    heading_m = re.search(r'#\s*Race\s*\d+\s*Analysis\s*(?:—|-|–)\s*(.+?)(?:\n|$)', text)
    if heading_m:
        info['race_name'] = heading_m.group(1).strip()
        
    # Then try explicit 賽事名稱
        ...
        for line in lines:
            ...
            if len(cols) >= 3:
                try:
                    ...
                    win_prob = float(win_prob_str.group(1)) if win_prob_str else 0
                    results.append({
                        'mc_rank': rank_idx, 'horse_num': 0,
                        'name': horse_name, 'win_prob': win_prob,
                        'predicted_odds': round(100 / win_prob, 2) if win_prob > 0 else 0,
                        'original_rank': None, 'agreement': '',
                    })
                except (ValueError, IndexError):
                    continue
    return results
...
def parse_hkjc_analysis(filepath: str) -> Optional[RaceAnalysis]:
    ...
    # Ensure TopPicks grades match actual parsed horse final_grades
    # This fixes Heison's markdown where it systematically outputs [A] correctly or incorrectly
    for pick in top_picks:
        h = next((h for h in horses if h.horse_number == pick.horse_number), None)
        if h and h.final_grade:
            pick.grade = h.final_grade

    # Fix: Re-sort Top Picks strictly by their final grades to ensure correct ranking order
    # (Matches user expectation that Top 4 panel strictly reflects the top 4 graded horses)
    GRADE_ORDER = ['S+', 'S', 'S-', 'A+', 'A', 'A-', 'B+', 'B', 'B-', 'C+', 'C', 'C-', 'D+', 'D', 'D-']
    
    def get_sort_key(horse):
        g_idx = GRADE_ORDER.index(horse.final_grade) if horse.final_grade in GRADE_ORDER else 99
        original_rank = next((p.rank for p in top_picks if p.horse_number == horse.horse_number), 99)
        return (g_idx, original_rank)

    sorted_all_horses = sorted(horses, key=get_sort_key)
    new_top_4 = sorted_all_horses[:4]
    
    new_picks = []
    for i, h in enumerate(new_top_4):
        # preserve original top pick reasoning if available
        orig = next((p for p in top_picks if p.horse_number == h.horse_number), None)
        if orig:
            orig.rank = i + 1
            new_picks.append(orig)
        else:
