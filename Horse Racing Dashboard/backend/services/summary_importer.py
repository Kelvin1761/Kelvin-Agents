...
def _normalize_date(date_str: str) -> str:
    ...
    Fixes:
    1. "2026-12-03" → "2025-12-03" (Dec 2026 hasn't happened yet, typo in source)
    2. "20.12", "23.12", "27.12" → these are D.M (day.month) format for December 2025
       Normal M.D dates have month ≤ 12, so if first part > 12, it's D.M format.
    
    HK racing season: ~Sep-Jul. November/December dates belong to the prior calendar year.
    """
    if not date_str:
        return date_str
        ...
        if len(parts) == 2:
            try:
                ...
                if a > 12 and b <= 12:
                    ...
                    # December -> year 2025 (racing season crosses year boundary)
                    month = b
                    day = a
...
def _parse_roi_sheet(sheet, region: str) -> List[Dict]:
    ...
    for r in range(header_row + 1, table.num_rows):
        ...
        venue = _safe_str(_read_cell(table, r, venue_col))
