...
def _race_summary(race):
    ...
    return {
        ...
        # Dual-track fields for betting panel
        "is_dual_track": race.is_dual_track,
        "primary_condition": race.primary_condition,
        "alt_condition": race.alt_condition,
        "alt_top_picks": [
            {
                "rank": p.rank,
                "horse_number": p.horse_number,
            ...
            for p in (race.alt_top_picks or [])
        ],
        "horses": [
            {
...
def _race_detail(race):
    ...
    return {
        ...
        # Dual-track scenario (SIP-1)
        "is_dual_track": race.is_dual_track,
        "primary_condition": race.primary_condition,
        "alt_condition": race.alt_condition,
        "alt_top_picks": [p.dict() for p in race.alt_top_picks] if race.alt_top_picks else [],
    }
