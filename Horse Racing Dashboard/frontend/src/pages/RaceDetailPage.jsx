  ...
  return (
    <div>
      ...
      {analysis?.battlefield_overview && (() => {
        ...
        // Split into table rows and non-table content
        ...
        const groupColors = {
          "領放": "#DC2626", "前中": "#D97706", "中後": "#2563EB", "後上": "#059669"
        };
        ...
        return (
          ...
          >
            ...
            <div style={{ padding: "12px 16px" }}>
              ...
              {tableRows.length > 0 && (
                <table style={{
                  width: "100%", fontSize: "0.78rem", borderCollapse: "collapse",
                  marginBottom: speedMapLines.length > 0 ? "12px" : "0",
                }}>
                  <tbody>
                    {tableRows.map((cells, i) => (
                      <tr key={i} style={{ borderBottom: "1px solid #F1F5F9" }}>
                        <td style={{
                          ...
                          width: "120px", background: i % 2 === 0 ? "#F8FAFC" : "#fff",
                          whiteSpace: "nowrap",
                        }}>{cells[0]}</td>
                        <td style={{
                          padding: "6px 10px", color: "#475569",
                          background: i % 2 === 0 ? "#F8FAFC" : "#fff",
                        }}>{cells.slice(1).join(" | ")}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {/* Speed Map */}
              {speedMapLines.length > 0 && (
                <div style={{
                  background: "#F8FAFC", borderRadius: "8px", padding: "10px 12px",
                  marginBottom: otherLines.length > 0 ? "8px" : "0",
                }}>
                  <div style={{ fontSize: "0.72rem", fontWeight: 700, color: "#64748B", marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                    📍 Speed Map
                  </div>
                  {speedMapLines.map((line, i) => {
                    const groupMatch = Object.keys(groupColors).find(g => line.includes(g));
                    return (
                      <div key={i} style={{ fontSize: "0.75rem", padding: "3px 0", color: "#334155", display: "flex", gap: "6px", alignItems: "flex-start" }}>
                        <span style={{
                          display: "inline-block", width: "6px", height: "6px", borderRadius: "50%",
                          background: groupMatch ? groupColors[groupMatch] : "#94A3B8",
    ...
    <div className="mc-panel" style={{ marginBottom: "24px" }}>
      ...
      <div className="mc-panel__body">
        <table className="mc-panel__table">
          ...
          <tbody>
            {results.map((r, i) => {
              const barWidth = maxProb > 0 ? (r.win_prob / maxProb) * 100 : 0;
              const rankIcons = ["🥇", "🥈", "🥉", "🏅"];
              ...
              return (
                <tr key={r.horse_num || i} className={i < 4 ? "mc-panel__row--top" : ""}>
                  ...
                  <td style={{ fontFamily: "monospace", color: "#0F766E" }}>
                    {r.predicted_odds ? `$${r.predicted_odds.toFixed(2)}` : "—"}
                  </td>
                  <td>{origIcon}</td>
                  <td>
                    <span
        ...
        <p className="mc-panel__note">
          🐍 由 Python <code>run_monte_carlo.py</code> 自動計算 · 基於 L400 段速 + EEM 能量
        </p>
      </div>
    </div>
  );
