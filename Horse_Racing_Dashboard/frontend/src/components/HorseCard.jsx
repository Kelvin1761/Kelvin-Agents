      ...
      <div className="horse-card__body">
        ...
        {expanded && hasAnalysis && (
          <div className="analysis-sections">
            {horse.raw_text ? (
              <StructuredAnalysisRenderer text={horse.raw_text} />
            ) : (
              <>
                {horse.speed_forensics && (
                  <AnalysisSection
                ...
                {horse.form_line && (
                  <div className="analysis-section">
                    <div className="analysis-section__header analysis-section__header--slate">
                      🔗 賽績線
                    </div>
...
/* ═══════════════════════════════════════════════════════════════
   AnalysisSection — Reusable section card with gradient header
   ═══════════════════════════════════════════════════════════════ */
function AnalysisSection({ icon, title, colorTheme, content }) {
  if (!content) return null;

  const formatted = formatAnalysis(content);
  const lines = formatted.split("\n").filter(l => l.trim().length > 0);

  return (
    <div className="analysis-section">
      <div className={`analysis-section__header analysis-section__header--${colorTheme}`}>
        {icon} {title}
      </div>
      <div className="analysis-section__body">
        {lines.map((line, i) => {
          ...
          if (trimmed.startsWith("•") || trimmed.startsWith("-")) {
            return (
              <div key={i} className="analysis-section__bullet">
...
/* ═══════════════════════════════════════════════════════════════
   StructuredAnalysisRenderer — Parse raw_text into section cards
   Detects section headings and splits into structured blocks
   ═══════════════════════════════════════════════════════════════ */
function StructuredAnalysisRenderer({ text }) {
  if (!text) return null;

  // Section detection patterns with their display config
  const sectionPatterns = [
    ...
    { pattern: /(?:💡|結論|Conclusion|Verdict|最終評級|綜合評級)/i, icon: "💡", title: "結論", theme: "emerald" },
    { pattern: /(?:📊|評級矩陣|Rating Matrix|Grading)/i, icon: "📊", title: "評級矩陣", theme: "blue" },
    { pattern: /(?:⚠️|風險|Risk|警號)/i, icon: "⚠️", title: "風險分析", theme: "rose" },
    { pattern: /(?:✅|優勢|Advantage|強項)/i, icon: "✅", title: "優勢分析", theme: "emerald" },
  ];

  // Split raw text into sections by detecting heading-like lines
  ...
  const isHeadingLine = (line) => {
    ...
    // Lines that look like section headers (short, bold, with emojis)
    if (trimmed.length < 60 && /^(?:\*{2}|[🔬⚡📋🔗🐴🧠💡📊⚠️✅🏎️🎯])/.test(trimmed)) return true;
    // Numbered sections like "Step 1:" or "第一步:"
    if (/^(?:Step \d|第[一二三四五六七八]|Part \d)/i.test(trimmed)) return true;
    return false;
  };

  const detectSection = (line) => {
    const clean = line.replace(/^#{1,4}\s*/, "").replace(/\*{1,2}/g, "").trim();
    for (const sp of sectionPatterns) {
      if (sp.pattern.test(clean)) return sp;
    }
    return null;
  };
  ...
  const flushSection = () => {
    if (currentLines.length > 0) {
      ...
      if (content) {
        sections.push({
          ...(currentSection || { icon: "📋", title: "分析", theme: "slate" }),
          content,
        });
      }
    }
  ...
  for (const line of lines) {
    ...
    if (/^[-=]{3,}$/.test(trimmed) || !trimmed) {
      if (currentLines.length > 0 && !trimmed) currentLines.push("");
      continue;
    }
  ...
  // If we couldn't detect any structured sections, fall back to table-aware rendering
  if (sections.length <= 1 && text.includes("|")) {
    return <SmartAnalysisRenderer text={text} />;
  }

  // If only one section, also fall back but with better formatting
  if (sections.length <= 1) {
    const content = sections[0]?.content || text;
    return (
      <AnalysisSection
        icon="📋"
        title="完整分析"
  ...
  return (
    <>
      {sections.map((section, i) => {
        // Special handling for conclusion sections
        if (section.theme === "emerald" && section.title === "結論") {
          return (
            <div key={i} className="analysis-conclusion">
              💡 <strong>結論:</strong>{" "}
              {formatAnalysis(section.content)}
            </div>
          );
        }
          ...
          if (tableLines.length >= 3) {
            return (
              <div key={i} className="analysis-section">
                <div className={`analysis-section__header analysis-section__header--${section.theme}`}>
                  {section.icon} {section.title}
                </div>
                <div className="analysis-section__body">
                  <SmartAnalysisRenderer text={section.content} />
                </div>
              </div>
            );
          }
